...
// Generated from the algebraic definition of the S-box - multiplicative
// inverse in GF(2^8) followed by the affine transform - rather than copied
// from a reference, and pinned by the FIPS-197 vectors in the tests.
...
void aes128_expand_key(const uint8_t key[AES128_KEY_SIZE],
  ...
  for (int i = 4; i < 44; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      round_keys[i * 4 + j] = (uint8_t)(round_keys[(i - 4) * 4 + j] ^ t[j]);
    }
  }
}
